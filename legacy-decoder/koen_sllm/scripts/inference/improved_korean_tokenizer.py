...
class Gemma3TokenizerWrapper:
    ...
    def __init__(self, model_name: str = "google/gemma-2-2b"):
        ...
        try:
            ...
            raise RuntimeError(f"Gemma3 토크나이저 로드 실패: {e}")
...
if __name__ == "__main__":
    ...
    # Gemma3 토크나이저 테스트 (선택적)
    try:
        print(f"\n" + "=" * 40)
        print("🤖 Gemma3 토크나이저 테스트")
        print("=" * 40)
        
        gemma_tokenizer = Gemma3TokenizerWrapper()
        
        test_text = "안녕하세요, Gemma3 토크나이저 테스트입니다."
        tokens = gemma_tokenizer.tokenize(test_text)
        encoded = gemma_tokenizer.encode(test_text)
        decoded = gemma_tokenizer.decode(encoded['input_ids'])
        ...
        print(f"⚠️ Gemma3 토크나이저 테스트 실패: {e}")
        print("  (transformers 라이브러리가 필요하거나 인터넷 연결이 필요할 수 있습니다)")
