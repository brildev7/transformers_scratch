#!/usr/bin/env python3
"""
Dynamic English dataset download script
JSON 설정 기반 영어 데이터셋 다운로드 스크립트
"""
import sys
import argparse
import logging
...
def main():
    parser = argparse.ArgumentParser(description="JSON 설정 기반 영어 데이터셋 다운로드")
    ...
    try:
        ...
        if not config_file.exists():
            ...
            logger.error(f"ls -la {project_root / 'common' / 'scripts' / 'datasets'}/")
            return 1
        ...
        output_filename = loader.global_settings.get('output_filename', 'english_corpus.json')
        english_path = output_dir / output_filename
        
        # 기존 파일 체크
        if english_path.exists() and not args.force:
            logger.info(f"기존 영어 데이터가 있습니다: {english_path}")
            logger.info("새로 다운로드하려면 --force 옵션을 사용하세요.")
            logger.info("또는 --info 옵션으로 설정 정보를 확인하세요.")
            return 0
        
        if args.force and english_path.exists():
        ...
        # 모든 데이터셋 다운로드
        english_texts = loader.load_all_datasets(small_mode=args.small)
        ...
        # 데이터 저장
        saved_file = loader.save_texts(english_texts, str(english_path))
        
        logger.info(f"✅ 영어 데이터셋 다운로드 완료!")
        logger.info(f"   - 문서 수: {len(english_texts):,}개")
